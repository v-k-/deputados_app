    // ++ ++ ++ NOTES:
    //////
    // - erro pra qd nao vem detalhes nao ficar tentando pra sempre
    // - outros erros   pra não bloquear
    // - tem q rever também a funçã retriever, pq acho q nao tá bem pensada pra
    ...
    // improved using fileURLToPath instead of new url().
    // this way it is safe for windows or any sys dev/t
    import { fileURLToPath } from "url";
    const __dirname = path.dirname(fileURLToPath(import.meta.url));

    //generic image
    const missingPhotoPath = path.join(
        __dirname,
        "public",
        "images",
        "appAssets",
    ...
    async function testAPI() {
        try {
            await axios.get(apiDeputados, { timeout: 10000, params: { itens: 1 } });
            console.log("API is up.");
            return true;
        } catch (error) {
            console.error(
    ...
    async function fetchPartidos() {
        try {
            ...
            return {}; // Ensure an empty obj is returned in case of error
        }
    }
    ...
    async function updateData() {
        ...
        try {
            ...
            if (thereIsData && thereIsDate && thereIsPartidos) {
                ...
                const lastUpdateDateData = await fs.promises.readFile(
                    "lastUpdateDate.json",
                );
                ...
                if (testMode) {
                    console.log(
                        `TESTMODE_ON: Serving file data from ${lastUpdateDate}`,
                    );
            ...
            if (await testAPI()) {
                ...
                mainTimer = setInterval(updateData, interval);
                console.log("Will refresh in 2 hs");
            } else {
                console.log("API is down, serving saved data. Will retry in half an hour");
                clearInterval(mainTimer);
                mainTimer = setInterval(updateData, 30 * 60 * 1000);// half an hour
            }
        } catch (error) {
    ...
    async function getApiData(url, params) {
        ...
        // this pattern is new for me, but efficient
        ...
        const fetchAllPages = async (url, params) => {
            try {
                ...
                //get the pg number using URL
                const lastPage = parseInt(
                    new URL(lastPageUrl).searchParams.get("pagina"),
                );
        ...
        for (const deputado of deputados) {
            try {
                ...
                if (deputado.id) {
                    ...
                    console.error(
                        `Deputado ${deputado.nome || "unknown"} has neither ID nor name.`,
                    );
                    continue;
                }
                ...
                if (deputado.id) {
                    ...
                    const fetchedDeputado = response.data.dados.find(
                        (dep) => dep.nome === deputado.nome,
                    );
                    if (fetchedDeputado) {
                        ...
                        console.error(
                            `Deputado ${deputado.nome} details could not be found.`,
                        );
                        continue;
                    }
                }
                ...
                console.error(
                    `Error populating details for deputado ${deputado.nome || deputado.id}:`,
                );
                if (error.response) {
    ...
    async function getImages(deputados) {
        ...
        for (const deputado of deputados) {
            ...
            if (deputado.id === "220593" && throwFetchError)
                url = deputado.urlFoto + "Fake error -  numfa";
    ...
    async function retriever() {
        ...
        const missingDetailsDeputados = _.filter(
            servingData,
            (dep) => !dep.details,
        );
        ...
        if (
            missingImagesDeputados.length > 0 ||
            (missingDetailsDeputados.length > 0 && retries < 5)
        ) {
            console.log(
                `there are ${missingImagesDeputados.length} missing images and ${missingDetailsDeputados.length} missing details to retrieve.`,
            );
            console.log(`attempting to retrieve image for ids`, missingImagesIds);
            console.log(
                `attempting to retrieve details for ids`,
                missingDetailsIds,
            );
            retries++;
            ...
            missingImagesDeputados.length > 0 ||
            missingDetailsDeputados.length > 0
        ) {
            console.log(
                `there are ${missingImagesDeputados.length} missing images and ${missingDetailsDeputados.length} missing details to retrieve.`,
            );
            console.log(
                `But the data is not available, will retry in next dataUpdate`,
            );
            ...
            console.log(
                `Dados baixados ${timesSaved++} ${singplural} desde que o servidor iniciou.`,
            );
            return;
        } else {
